...
function getSettings() {
  if (Date.now() - lastSettingsFetch > 5000) { // fetch every 5s max
    db.get("SELECT value FROM sensor_config WHERE key = 'app_settings'", (err, row) => {
      if (row) {
        try { cachedSettings = JSON.parse(row.value); } catch(e){}
      }
    });
...
module.exports = function buildTelemetryRouter(io) {
  ...
  router.post("/", requireDeviceKey, (req, res) => {
    ...
    const point = {
      ...
      gps_lat: numOrNull(b.gps_lat),
      gps_lon: numOrNull(b.gps_lon),
      gps_sats: numOrNull(b.gps_sats),
    };

    db.run(`INSERT INTO telemetry 
      (mission_id, timestamp, temperature, humidity, mq135, mq9, flame, 
       accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z, 
       altitude, velocity, battery, gps_lat, gps_lon, gps_sats) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [missionId, new Date(timestamp).toISOString(), point.temperature, point.humidity, point.mq135, point.mq09, point.flame,
       point.accel_x, point.accel_y, point.accel_z, point.gyro_x, point.gyro_y, point.gyro_z,
       point.altitude, point.velocity, point.battery, point.gps_lat, point.gps_lon, point.gps_sats],
      function(err) {
        if (err) {
        ...
        if (point.flame === 1 && (nowMs - lastAlerts.flame > 5000)) {
          ...
          db.run("INSERT INTO alerts (mission_id, severity, sensor, message) VALUES (?, ?, ?, ?)", [missionId, 'CRITICAL', 'Flame', 'Flame detected']);
        }
        
        if (point.mq09 !== null && point.mq09 >= (s.thresholds?.mq09?.critical || 800) && (nowMs - lastAlerts.mq09 > 5000)) {
          lastAlerts.mq09 = nowMs;
          db.run("INSERT INTO alerts (mission_id, severity, sensor, message) VALUES (?, ?, ?, ?)", [missionId, 'CRITICAL', 'MQ-09', 'Critical gas level']);
        }
        
        if (point.mq135 !== null && point.mq135 >= (s.thresholds?.mq135?.critical || 800) && (nowMs - lastAlerts.mq135 > 5000)) {
          lastAlerts.mq135 = nowMs;
          db.run("INSERT INTO alerts (mission_id, severity, sensor, message) VALUES (?, ?, ?, ?)", [missionId, 'CRITICAL', 'MQ-135', 'Critical gas level']);
        }
      }
    );
  });
  ...
  router.get("/latest", requireAuth, (req, res) => {
    ...
    db.get("SELECT * FROM telemetry WHERE mission_id = ? ORDER BY timestamp DESC LIMIT 1", [missionId], (err, row) => {
  ...
  router.get("/history", requireAuth, (req, res) => {
    ...
    db.all("SELECT * FROM telemetry WHERE mission_id = ? ORDER BY timestamp DESC LIMIT ?", [missionId, limit], (err, rows) => {
      if (err) return res.status(500).json({ error: "DB error" });
